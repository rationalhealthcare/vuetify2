...
export const ConsultantAdapter = {
  searchOrganizations: async function(payload) {
    const OrganizationSearchObject = function(obj) {
      this.otherorgname = obj.facilityname === null ? "" : obj.facilityname;
    ...
    let res = await axios.post(
      BASE_URL + NPI_ORGANIZATION_SEARCH_ENDOINT,
      search,
      AUTH_HEADER
    );
    if (res) {
  ...
  searchPractitioners: async function(payload) {
    if (payload) {
      const MyPractitioner = function(payload) {
        this.firstname = payload.firstname === null ? "" : payload.firstname;
        this.lastname = payload.lastname === null ? "" : payload.lastname;
        this.city = payload.city === null ? "" : payload.city;
        this.state = payload.state === null ? "" : payload.state;
        this.postalcode = payload.postalcode === null ? "" : payload.postalcode;
      };
      let search = new MyPractitioner(payload);
      console.log("Search object", search);
      let res = await axios.post(
        BASE_URL + NPI_PRACTITIONER_SEARCH_ENDOINT,
        search,
        AUTH_HEADER
      );
      if (res) {
  ...
  searchContactDetails: async function(payload) {
    ...
    npi.push(payload);
    let res = await axios.post(
      BASE_URL + NPI_ADDRESSES_SEARCH_ENDOINT,
      npi,
      AUTH_HEADER
    );
    if (res) {
